...
Das Projekt ***QTCoffeeSlotMachine*** ist eine Vorlage für die Erstellung von datenzentrierten Anwendungen. Ausgehend von dieser Vorlage  
...
4. [Verwendung der Vorlage](#verwendung-der-Vorlage)  
   ...
   2. [Abgleich mit dem QTCoffeeSlotMachine](#abgleich-mit-dem-QTCoffeeSlotMachine)  
...
## Template  
Die Struktur vom 'QTCoffeeSlotMachine' besteht aus unterschiedlichen Teilprojekten und diese in einer Gesamtlösung (im Kontext von Visual Studio ist das eine Solution) zusammengefasst. Eine Erläuterung der einzelnen Projekte, deren Typ und die Abhängigkeit finden sie in der folgenden Tabelle:  
...
|**QTCoffeeSlotMachine.Logic**|Dieses Projekt beinhaltet den vollständigen Datenzugriff, die gesamte Geschäftslogik und stellt somit den zentralen Baustein des Systems dar.|Library|CommonBase|  
|**QTCoffeeSlotMachine.WebApi**|In diesem Projekt ist die REST-Schnittstelle implementiert. Diese Modul stellt eine API (Aplication Programming Interface) für den Zugriff auf das System über das Netzwerk zur Verfügung.|Host|CommonBase, QTCoffeeSlotMachine.Logic|  
|**QTCoffeeSlotMachine.ConApp**|Dieses Projekt dient als Initial-Anwendung zum Erstellen der Datenbank, das Anlegen von Anmeldedaten falls die Authentifizierung aktiv ist und zum Importieren von bestehenden Daten. Nach der Initialisierung wird diese Anwendung kaum verwendet.|Console|CommonBase, QTCoffeeSlotMachine.Logic|  
|**QTCoffeeSlotMachine.AspMvc**|Diese Projekt beinhaltet die Basisfunktionen für eine AspWeb-Anwendung und kann als Vorlage für die Entwicklung einer einer AspWeb-Anwendung mit dem QTCoffeeSlotMachine verwendet werden.|Host|CommonBase, QTCoffeeSlotMachine.Logic|  
|**QTCoffeeSlotMachine.WpfApp**|Diese Projekt beinhaltet die Basisfunktionen für eine Wpf-Anwendung und kann als Vorlage für die Entwicklung einer einer Wpf-Anwendung mit dem QTCoffeeSlotMachine Framework verwendet werden.|Host|CommonBase, QTCoffeeSlotMachine.Logic|  
|**QTCoffeeSlotMachine.XxxYyy**|Es folgen noch weitere Vorlagen von Client-Anwendungen wie Angular, Blazor und mobile Apps. Zum jetzigen Zeitpunkt existiert nur die AspMvc-Anwendung. Die Erstellung und Beschreibung der anderen Client-Anwendungen erfolgt zu einem späteren Zeitpunkt.|Host|CommonBase, QTCoffeeSlotMachine.Logic|  
...
Nachfolgend werden die einzelnen Schritte von der Vorlage ***QTCoffeeSlotMachine*** bis zum konkreten Projekt ***QTMusicStoreLight*** erläutert. Das Projekt ist eine einfache Anwendung zur Demonstration von der Verwendung der Vorlage. Im Projekt ***QTMusicStoreLight*** werden Künstler (Artisten) und deren produzierten Alben verwaltet. Jedes Album hat ein Genre (Rock, Pop, Klassik usw.) zugeordnet. Ein Datenmodell ist im nachfolgendem Abschnitt definiert.  
   
## System-Erstellungs-Prozess  
  
Wenn nun ein einfacher Service oder eine Anwendung entwickelt werden soll, dann kann die Vorlage ***QTCoffeeSlotMachine*** als Ausgangsbasis verwendet und weiterentwickelt werden. Dazu empfiehlt sich folgende Vorgangsweise:  
  
### Vorbereitungen  
  
- Erstellen eines Ordners (z.B.: Develop)  
- Herunterladen des Repositories ***QTCoffeeSlotMachine*** vom [GitHub](<https://github.com/leoggehrer/CSSoftwareEngineering-QTCoffeeSlotMachine>) und in einem Ordner speichern.  
  
> **ACHTUNG:** Der Solution-Ordner von der Vorlage muss ***QTCoffeeSlotMachine*** heißen.  
...
Als Ausgangsbasis wird die Vorlage ***QTCoffeeSlotMachine*** verwendet. Diese Vorlage wird mit Hilfe dem Hilfsprogramm ***'TemplateCopier.ConApp'*** in ein Verzeichnis eigener Wahl kopiert. In diesem Verzeichnis werden alle Projektteile (mit Ausnahme der Hilfsprogramme *TemplateCopier.ConApp* und *TemplateComparison.ConApp*) von der Vorlage kopiert und die Namen der Projekte und Komponenten werden entsprechend angepasst. Alle Projekte mit dem Prefix ***QTCoffeeSlotMachine*** werden mit dem domainspezifischen Namen des Verzeichnisses ersetzt. Beim Kopieren der Dateien von der Vorlage werden alle Dateien mit dem Label ***@CodeCopy*** durch den Label ***@CodeCopy*** ersetzt. Diese Label werden für den Abgleich-Prozess verwendet.  
...
Copy 'QTCoffeeSlotMachine' from: ...\source\repos\HtlLeo\CSSoftwareEngineering\QTCoffeeSlotMachine  
...
**Hinweis:** Die Vorlage muss im Ordner (*QTCoffeeSlotMachine*) gespeichert sein.  
...
Source: ...\source\repos\HtlLeo\CSSoftwareEngineering\QTCoffeeSlotMachine\  
...
Wird nun die Option **[1 oder a]** aktiviert, dann werden alle Dateien im Projekt **QTCoffeeSlotMachine** mit der Kennzeichnung **@BaseCopy** mit den Dateien im Projekt **QTMusicStoreLight** mit der Kennzeichnung **@CodeCopy** abgeglichen.  
